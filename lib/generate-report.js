...
export const generateReport = async (data, shouldDownload = true) => {
  ...
  try {
    const logoUrl = "/pv-logo.png";
    doc.addImage(logoUrl, 'PNG', 15, 10, 25.2, 19.2); // Moved down and aligned with grey box
  } catch(e){}

  // Grey Title Block
  doc.setFillColor(80, 80, 80); 
  doc.rect(15, 45, 180, 65, "F"); // Moved block down slightly for better page balance

  doc.setTextColor(255, 255, 255);
  doc.setFontSize(32);
  doc.setFont("helvetica", "bold");
  doc.text(surveyType, 22, 68); // Centered title better
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(230, 230, 230);
  doc.text(`Contact: ${mobile} | ${email}`, 22, 86);
